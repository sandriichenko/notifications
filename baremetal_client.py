...
#    not use this file except in compliance wgementith the License. You may obtain
...
class BaremetalClient(base.BaremetalClient):
    ...
    @base.handle_errors
    def set_maintenance(self, node_uuid, reason={}):
        """Set power state of the specified node.
        :param node_uuid: The unique identifier of the node.
        :state: desired state to set (on/off/reboot).
        """
        resp, body = self._put_request('nodes/%s/maintenance' % node_uuid,
                                       reason)
        self.expected_success(202, resp.status)
        return resp, body
